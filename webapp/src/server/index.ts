...
import { generateHagakiPDF } from './pdfService.js';
...
app.post('/api/generate-hagaki-pdf', async (req, res) => {
  try {
    ...
    if (!customerIds || !Array.isArray(customerIds)) {
      return res.status(400).json({ error: '顧客IDのリストが必要です' });
    }

    // 全顧客データを取得
    const allCustomers = await readCustomers();

    // 指定された顧客のみフィルタ（IDが指定されている場合）
    let targetCustomers = allCustomers;
    if (customerIds.length > 0 && customerIds[0] !== 'all') {
      targetCustomers = allCustomers.filter((c) =>
        customerIds.includes(c['顧客番号'])
      );
    }
    ...
    // PDF生成
    console.log(`${targetCustomers.length}件のはがきPDFを生成中...`);
    const pdfBuffer = await generateHagakiPDF(targetCustomers, senderInfo);

    // PDFを返す
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="hagaki_${new Date().toISOString().slice(0, 10)}.pdf"`
    );
    res.send(pdfBuffer);
  } catch (err) {
    console.error('PDF生成エラー:', err);
    res.status(500).json({ error: 'PDF生成に失敗しました', detail: String(err) });
  }
});
